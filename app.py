...
from data import db, User, Character
...
def characters_of_user(username):
    ...
    """
    user = db.session.query(User).filter_by(username=username).one_or_none()
    characters = data_manager.get_characters(user.user_id)
...
def add_character(username):
    ...
    character = data_manager.create_character(character, user.user_id)
...
def delete_character(username, char_name):
    ...
    """
    user = db.session.query(User).filter_by(username=username).one_or_none()
    char_to_delete = user.created_chars.filter(Character.char_name == char_name,
                                               user_id = user.user_id).one_or_none()

    data_manager.delete_character(char_to_delete.char_id)
